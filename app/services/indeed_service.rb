...
class IndeedService
  ...
  def execute(job1, job2, job_scale, location)
    search1 = format_request(job1, location)
    search2 = format_request(job2, location)

    @response = {
      'score'         => calculate_weighted_score(@client.search(search1)['totalResults'], @client.search(search2)['totalResults'], job_scale),
  ...
  def calculate_weighted_score(job_total_1, job_total_2, job_scale)
  ...
  def calculate_weight(job_scale)
    return [job_scale, 100 - job_scale]
  end
